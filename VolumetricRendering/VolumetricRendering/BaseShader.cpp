...
BaseShader::BaseShader(const char* shaderPath)
{
	...
	m_shader = glCreateShader(m_shaderType.type);
	glShaderSource(m_shader, 1, &shaderString, NULL);
	glCompileShader(m_shader);
	checkCompileErrors(m_shader, m_shaderType.name.c_str(), getShaderName(shaderPath));

}

std::string BaseShader::getName()
{
	return m_shaderType.name;
}

ShaderType BaseShader::getType()
{
	return m_shaderType;
}

unsigned int BaseShader::getShader()
{
	return m_shader;
}
